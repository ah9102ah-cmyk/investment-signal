...
个人投资参考网页 + 每日信号/持仓邮件系统。静态页部署在 GitHub Pages,数据由云端 Actions 每日抓取 + 页面端实时行情组成。
...
investment-signal/
├── web/
│   ├── index.html              # 主页面(基金/股票 tab、三维度信号、基本面&目标价、资产明细)
│   ├── strategy.html           # 策略说明页
│   └── data/fund.json          # 基本面数据(每日云端重算, 页面主数据源)
├── scripts/
│   ├── fetch_fundamentals.py   # ★ 数据管道主脚本: 东财F10(财务) + 腾讯(实时PE/PB/K线) + akshare(港股财务/券商预期/指数估值) -> fund.json
...
│   ├── holdings_email.py       # 持仓建议邮件(云端 SMTP 模式, 爱基金持仓 -> 建议)
...
```
[东财 F10 财务] ──┐
[腾讯 实时行情] ──┼─> scripts/fetch_fundamentals.py ─> web/data/fund.json ──> GitHub Pages 托管
...
```bash
py scripts/fetch_fundamentals.py     # 重算 fund.json(写 scripts/data/ 和 web/data/ 两份)
...
- `ah9102ah-cmyk/holdings-email` — 持仓建议邮件(每日 12:00 北京, SMTP)
