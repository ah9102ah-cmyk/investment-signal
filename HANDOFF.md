...
> 2026-08-03 已完成本地复审和改造，仍在 `dev` 分支，**没有推送 GitHub、没有合并 main、没有触发云端 workflow**。下方旧交接内容保留作历史背景，其中旧目标价公式、旧动能方向和旧回测数字已经过时。
...
- 回测改为每周最后一个交易日观察、只纳入正动量资产、全负时现金，并计入单边 0.1% 换手成本。
- 推送辅助脚本固定只写 `dev`；过期的 `push_api.py` 已停用。未经用户明确同意仍不得运行任何推送。

本地验证结果：

- 8 项核心逻辑测试通过；所有修改过的 Python 文件和页面 JavaScript 均可解析。
- 公开数据重抓成功，16 项数据生成完成，根目录与 web 副本 SHA-256 一致。
- 可复现回测（2013—2026-08-03）：全程持有基准年化 6.6% / 最大回撤 -39.1%；双轮观察规则 5.6% / -16.5%；40 日动量轮动 5.7% / -34.2%。没有发现稳定超额收益，双轮规则主要体现为历史回撤较低。

正式上线仍需：用户过目 → 合并回 `main` → 手动触发一次 `update-fundamentals` workflow → 再确认次日 09:00 自动更新。当前线上版本不能视为已验证。
