...
静态投资参考网页(基金+股票自选,三维度信号、五维评分、目标价、实时行情),数据 = 每日 09:00 云端 Actions 抓取 + 浏览器端实时拉取。
...
2. **不要替用户下交易指令**。页面/邮件只给数据、评分、判断依据,结论用"持有为主/观望/谨慎"这类参考语气,不给买卖指令。
3. **判断必须"技术面 + 基本面估值"双轮**,不能只看单指标。
4. **文案要大白话**。页面和邮件用"最近在上涨/估值偏便宜/约X年回本"这种说法,不用"空头排列/RSI 背离"术语;红涨绿跌。
...
| `scripts/fetch_fundamentals.py` | 数据管道: F10 财务 → 五维评分/目标价 → fund.json | 改了要本地重跑生成 fund.json |
| `scripts/stocks.json` | 股票清单 | 加 A股用 6 位代码,港股用 5 位(hk 前缀在 index.html ASSETS) |
| `web/data/fund.json` | 页面主数据(每次跑脚本后更新) | 与 scripts/data/fund.json 同步 |

## 修改后的验证流程

1. `py scripts/fetch_fundamentals.py`(重算 fund.json,看输出有没有异常/降级)
2. `cd web && py -3 -m http.server 8765` 本地起服务
3. 无头浏览器打开 `http://127.0.0.1:8765` 检查: 控制台无 JS 报错、股票 tab 每只评分/目标价/评价正常、三维度 S = V+T+M
