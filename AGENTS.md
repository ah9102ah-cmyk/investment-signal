...
静态投资参考网页(基金+股票自选,三维度信号、最终三态结果、五维评分、12个月估值区间、实时行情),数据 = 每日 09:00 云端 Actions 抓取 + 浏览器端实时拉取。
...
2. **允许显示固定规则的“买入/持有/卖出”策略状态，但不要替用户下个性化交易指令**。三态必须同时展示判断依据；“持有”在未持有时解释为继续观望。不得读取用户账户后自动下单，不得把规则状态写成收益承诺。
...
3. 浏览器打开 `http://127.0.0.1:8765` 检查: 控制台无 JS 报错、股票 tab 每只评分/估值区间/评价正常、V 缺失时 S 显示“—”；当前基金观察项缺 V 时保守显示“持有”并标“估值缺失”，其他场景显示“数据不足”；其余 S = V+T+M，三态结果与统一规则层一致
